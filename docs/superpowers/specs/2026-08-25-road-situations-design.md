...
terminal departure node, which is left exactly as authored. Every node is a real
decision, so the subsystem adds nodes without adding page-turns: the ratchet's
...
| **the gate** | Arrival. What Farrukh says about who he passed, to an authority that is entitled to ask. |
...
2. **Displaced, not deserters.** An earlier draft made the strangers hashar levies who
   ...
   expects, and they are walking to a brother. Naming them at the gate exposes them to
...
5. **The terminal departure node is never touched.** It carries `next_chapter_id` and the
   chapter's last word. The road is inserted before it, so chapter routing, the journey
   map, and every test that asserts where a chapter ends are all unaffected.
...
| `named_them_at_the_gate` | answering the clerk fully | sarakhs `n07_a_quiet_request` |
...
an hour after Farrukh told a clerk with a board exactly who he passed on the road.
